...
extern int showPerspectiveView(View* view) {
    ...
    mtx = GRAPH_ALLOC_TYPE(g, Mtx, 1);
    view->p_viewing = mtx;
    guLookAt(mtx, view->eye.x, view->eye.y, view->eye.z, view->center.x, view->center.y, view->center.z, view->up.x,
             view->up.y, view->up.z);
...
extern int showView1(View* view, int flag_mask, Gfx** gfx_p) {
    ...
    if (flag_mask & VIEW_UPDATE_ORTHOGRAPHIC) {
        ...
        view->p_projection = m;
        guOrtho(m, -(f32)ScreenWidth / 2.0f, (f32)ScreenWidth / 2.0f, -(f32)ScreenHeight / 2.0f,
        ...
        view->p_projection = m;
        guPerspective(m, &view->normal, view->fovY,
    ...
    if (flag_mask & VIEW_UPDATE_LOOKAT) {
        ...
        view->p_viewing = m;
        guLookAt(m, view->eye.x, view->eye.y, view->eye.z, view->center.x, view->center.y, view->center.z, view->up.x,
                 view->up.y, view->up.z);
