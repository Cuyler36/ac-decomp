...
static void mSM_change_view(GRAPH* graph, f32 eye_dist, f32 y_lookAt, f32 vp_x, f32 vp_y, s16 angle, int width,
    ...
    if (!g_game_WidescreenEnabled) {
        ...
        if (width != 256) {
            guPerspective(mtx, &persp_norm, 35.0f, 16.0f / 9.0f, 1.0f, 2000.0f, 1.0f);
        } else {
            guPerspective(mtx, &persp_norm, 20.0f, 16.0f / 9.0f, 100.0f, 800.0f, 1.0f);
        }
    }
...
static void mSM_move_Play(Submenu* submenu) {
    ...
    /* Shit hack to make our change view proc without adding in the entire m_submenu_ovl TU */
    if (submenu->overlay->change_view_proc != &mSM_change_view) {
