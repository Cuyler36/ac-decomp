...
/* Count the number of unique bg blocks, O(n^2) time complexity */
static int mFM_CountUniqueBGData(const u16* const bg_ids, const int n, u16* const unique_order) {
    int count = 0;
    int i;
    int j;
...
static mFM_bg_data_c* mFM_LoadBGData(mFM_bg_data_c** const bg_block, const u16* const bg_ids, const int count) {
    char filepath[50];
    u16* unique_order = (u16*)zelda_malloc(sizeof(u16) * count);
    int unique_blocks = mFM_CountUniqueBGData(bg_ids, count, unique_order);
    mFM_bg_data_c* dst = (mFM_bg_data_c*)zelda_malloc_align(unique_blocks * sizeof(mFM_bg_data_c), 32);
    int i;
    int j;

    /* Load unique blocks into RAM */
    for (i = 0; i < unique_blocks; i++) {
        /* Convert the combination to file path */
        snprintf(filepath, sizeof(filepath), "/data/bg/%04x.bin", unique_order[i]);
        JC__JKRDvdToMainRam_byName(filepath, (u8*)&dst[i], EXPAND_SWITCH_DECOMPRESS);
    }

    /* Set the bg_block pointers */
    for (i = 0; i < count; i++) {
        for (j = 0; j < unique_blocks; j++) {
            if (bg_ids[i] == unique_order[j]) {
                bg_block[i] = &dst[j];
                break;
            }
        }
    ...
    /* Free unique_order, it's the caller's job to free dst */
    zelda_free(unique_order);
}
...
static void mFM_BlockDataSet2(const mActor_name_t field_name, mFM_block_info_c* block_info,
                              const mFM_combination_c* save_combi_table, mFM_fg_data_c** fg_data_list,
                              const mFM_combo_info_c* data_combi_table, const int bx_max, const int bz_max,
    ...
    bg_ids = zelda_malloc(max * sizeof(u16));
    if (bg_ids != NULL) {
        for (i = 0; i < max; i++) {
            combi = &data_combi_table[save_combi_table[0].combination_type];
            mFM_ChangeBGName(&bg_name, combi->bg_id, field_name);
        ...
        block_data = zelda_malloc(max * sizeof(mFM_bg_data_c*));
        if (block_data != NULL) {
            alloc_bg_data = mFM_LoadBGData(block_data, bg_ids, max);
            if (alloc_bg_data != NULL) {
                ...
                for (i = 0; i < max; i++) {
                    ...
                    if (fg_data_list[fg_idx]) {
                        mFM_SetBG(&block_info->bg_info, block_data[i], save_combi_table[0].height, combi->type, bg_name,
                                  bx, bz);
                        mFM_SetFG(&block_info->fg_info, fg_data_list[fg_idx], fg_name);
                    }
                ...
                zelda_free(alloc_bg_data);
            }
            zelda_free(block_data);
        }
        zelda_free(bg_ids);
    }
}
...
static int mFM_SetBlockInfo2(mFM_fdinfo_c* field_info, mFM_combination_c* combi_table) {
    mFM_fg_data_c** sorted_fg_data;
    mFM_combo_info_c* combi_info_p;
    mFM_bg_data_c* bg_data_p = data_bgd;
    mFM_fg_data_c* fg_data;
    ...
    if (fg_data != NULL) {
        ...
        if (sorted_fg_data != NULL) {
            mFM_SortFGData(sorted_fg_data, fg_data, fg_count, fg_data_entries, name_start);
            mFM_BlockDataSet2(field_info->field_id, field_info->block_info, combi_table, sorted_fg_data, combi_info_p,
                              field_info->block_x_max, field_info->block_z_max, name_start);

            mFM_SetFG2(field_info->fg2_p[0], sorted_fg_data, field_info->field_id, name_start);

            if (mFI_GET_TYPE(field_info->field_id) == mFI_FIELD_NPCROOM0) {
                // select a random furniture item from NPC base furniture layer
                mNpc_SetNpcFurnitureRandom(sorted_fg_data, name_start);
            }
...
extern void mFM_RenewalReserve() {
    ...
    for (bz = 1; bz < FG_BLOCK_Z_NUM; bz++) {
        for (bx = 0; bx < FG_BLOCK_X_NUM; bx++) {
            mFM_RenewalReserveBlock(fg_block->items[0], &set, &set_idx, &river_idx,
                                    g_block_kind_p[(bz + 1) * BLOCK_X_NUM + (bx + 1)]);
            fg_block++;
        }
    }
}
